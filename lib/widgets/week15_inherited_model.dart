...
class ColorWidget1 extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      height: 100,
      width: 100,
      decoration: BoxDecoration(borderRadius: BorderRadius.circular(24), color: ancestor.colorOne),
    );
  }
}
...
class ColorWidget2 extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    final ancestor = InheritedModel.inheritFrom<MyAncestor>(context, aspect: 'two');
    return Container(
      height: 100,
      width: 100,
...
class MyAncestor extends InheritedModel<String> {
  ...
  bool updateShouldNotifyDependent(MyAncestor oldWidget, Set<String> dependencies) {
    if (dependencies.contains('one') && colorOne != oldWidget.colorOne) return true;
    if (dependencies.contains('two') && colorTwo != oldWidget.colorTwo) return true;
    return false;
  }
}
