      ...
      children: <Widget>[
        ...
        new Row(
          children: <Widget>[
            ...
            new RaisedButton(textColor: Colors.black, child: new Text('查'), onPressed: _query),
          ],
        ),
        new Padding(
          padding: const EdgeInsets.only(top: 10.0, bottom: 10.0),
...
class _HandleSQLiteDataWidgetState extends State<HandleSQLiteDataWidget> {
  ...
  _update() async {
    ...
    int count = await db.rawUpdate(sql, ['$pwd', '$username']);
    print(count);
    await db.close();
    setState(() {
      _result = "更新数据成功，请查看";
  ...
  _queryByUsername() async {
    String sql_query_user = 'SELECT * FROM user_table WHERE username = ?';
    Database db = await openDatabase(dbPath);
    List<Map> list = await db.rawQuery(sql_query_user, ['$queryUsername']);
    var user = list.first;
    await db.close();
    setState(() {
      _result = "数据详情：$user";
...
//            new RaisedButton(textColor: Colors.black, child: new Text('创建'), onPressed: _create),
        ...
        new Row(
          children: <Widget>[
            ...
            new RaisedButton(textColor: Colors.black, child: new Text('查一个'), onPressed: _queryByUsername),
          ],
        ),
        new Column(
          children: <Widget>[
    ...
    return new Column(
      children: <Widget>[
        new TextField(
          ...
          obscureText: false,
          maxLines: 1,
          decoration: new InputDecoration(labelText: '用户名', hintText: '请输入英文或数字'),
        ...
        new TextField(
          ...
          obscureText: true,
          maxLines: 1,
          decoration: new InputDecoration(hintText: '请输入长度大于6的密码', labelText: '密码'),
          keyboardType: TextInputType.text,
          onSubmitted: (text) {},
        ),
      ],
