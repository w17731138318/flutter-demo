...
import 'package:flutter_widget_of_the_week/sqflite_demo/database/database.dart';
  ...
  RawTestPage() : super("Raw tests") {
      ...
      try {
        ...
        Future _test(int i) async {
          await db.transaction((txn) async {
            int count = Sqflite.firstIntValue(await txn.rawQuery("SELECT COUNT(*) FROM Test"));
            await Future.delayed(Duration(milliseconds: 40));
            await txn.rawInsert("INSERT INTO Test (name) VALUES (?)", ["item $i"]);
            //print(await db.query("SELECT COUNT(*) FROM Test"));
            int afterCount = Sqflite.firstIntValue(await txn.rawQuery("SELECT COUNT(*) FROM Test"));
    ...
    test("Concurrency 1", () async {
      ...
      try {
        ...
        Future action1() async {
          ...
          await step2.future;
          try {
            await db.rawQuery("SELECT COUNT(*) FROM Test").timeout(Duration(seconds: 1));
            throw "should fail";
          } catch (e) {
        ...
        int count = Sqflite.firstIntValue(await db.rawQuery("SELECT COUNT(*) FROM Test"));
        expect(count, 1);
      } finally {
        await db?.close();
      }
    ...
    test("Transaction open twice", () async {
      ...
      try {
        ...
        */
        ...
        int db2AfterCount = Sqflite.firstIntValue(await db2.rawQuery("SELECT COUNT(*) FROM Test"));
    ...
    test("Demo", () async {
      ...
      try {
        ...
        print("table created");
        int id = await database.rawInsert('INSERT INTO Test(name, value, num) VALUES("some name",1234,?)', [456.789]);
        print("inserted1: $id");
        id = await database.rawInsert('INSERT INTO Test(name, value) VALUES(?, ?)', ["another name", 12345678]);
        print("inserted2: $id");
    ...
    test("Open twice", () async {
      ...
      try {
        ...
        int count = Sqflite.firstIntValue(await db2.rawQuery("SELECT COUNT(*) FROM Test"));
        expect(count, 0);
      } finally {
        await db?.close();
        await db2?.close();
    ...
    test("without rowid", () async {
      ...
      try {
        ...
        // a rowid
        await db.execute("CREATE TABLE Test (name TEXT PRIMARY KEY) WITHOUT ROWID");
        int id = await db.insert("Test", {"name": "test"});
    ...
    test('Reference query', () async {
      ...
      try {
        ...
        batch.execute("CREATE TABLE Test (id INTEGER PRIMARY KEY, name TEXT, other REFERENCES Other(id))");
        batch.rawInsert("INSERT INTO Other (name) VALUES (?)", ["other 1"]);
        batch.rawInsert("INSERT INTO Test (other, name) VALUES (?, ?)", [1, "item 2"]);
        await batch.commit();

        var result = await db.query('Test', columns: ['other', 'name'], where: 'other = 1');
        print(result);
        expect(result, [
          {"other": 1, "name": "item 2"}
        ]);
        result = await db.query('Test', columns: ['other', 'name'], where: 'other = ?', whereArgs: [1]);
        print(result);
        expect(result, [
          {"other": 1, "name": "item 2"}
        ]);
